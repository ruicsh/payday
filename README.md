...
| `region`                   | string or null      | Where you **live/work** for Income Tax: `"scotland"` = Scottish Income Tax; `"england"`/`"wales"`/`"northern_ireland"`/`"rest_of_uk"` (or `null`) = rUK rates. Non-Scottish aliases are equivalent to `rest_of_uk`. This does **not** determine your student-loan plan — that is set by where you were *funded* (see [Student loan plans](#student-loan-plans)). |
...
| `student_loan_plan`        | string or null      | Undergraduate plan: `"plan1"`, `"plan2"`, `"plan4"`, `"plan5"` (or `null` = no loan). Pick the plan that matches your **funding body** and course start date — see [Student loan plans](#student-loan-plans) (e.g. SAAS-funded = `plan4`, Student Finance NI = `plan1`). All modes — PAYE/Inside IR35 via PAYE, Outside IR35 via Self Assessment on salary+dividends, Sole Trader via Self Assessment on taxable profit + existing. Scotland and NI **postgraduate** loans are not a separate plan: they repay under `"plan4"` (Scotland) and `"plan1"` (NI) at 9%, combined with the undergraduate debt. Lifelong Learning Entitlement (from Jan 2027) and Advanced Learner Loans also use existing plans: `"plan5"` (post-Aug-2023) or `"plan2"` (pre-2023). |
| `postgraduate_loan`        | bool or null        | `true` = Postgraduate Loan (**Plan 3**, England & Wales only — 6% above £21,000); stacks on top of `student_loan_plan`. `false`/`null` = none. Independent — valid without an undergraduate plan. Postgraduate loans funded by **Scotland**/**Northern Ireland** are not Plan 3 — they are covered by `student_loan_plan: "plan4"` / `"plan1"` instead. |

### Student loan plans

Your repayment plan is fixed by **which funding body lent you the loan and when your course started** — it does *not* depend on where you now live or work. A Scottish-funded graduate working in London is still Plan 4; an English-funded graduate living in Scotland is not. Choose `student_loan_plan` from the funding-body row that matches you:

| Funding body | Undergraduate | Postgraduate (Master's/Doctoral) |
|---|---|---|
| **Scotland** — Student Awards Agency Scotland (SAAS) | Plan 4 | Plan 4 (no separate Plan 3) |
| **Northern Ireland** — Student Finance NI | Plan 1 | Plan 1 (no separate Plan 3) |
| **England** — Student Finance England, started before 1 Sep 2012 | Plan 1 | — |
| **England** — Student Finance England, started 1 Sep 2012 – 31 Jul 2023 | Plan 2 | Postgraduate Loan (Plan 3) |
| **England** — Student Finance England, started on/after 1 Aug 2023 | Plan 5 | Postgraduate Loan (Plan 3) |
| **Wales** — Student Finance Wales, started before 1 Sep 2012 | Plan 1 | — |
| **Wales** — Student Finance Wales, started on/after 1 Sep 2012 | Plan 2 | Postgraduate Loan (Plan 3) |

All undergraduate plans (1/2/4/5) repay **9%** above their thresholds (£26,900 / £29,385 / £33,795 / £25,000 in 2026/27). The Postgraduate Loan (**Plan 3**) repays **6%** above £21,000 and stacks on top of any undergraduate plan — but it is **England & Wales only**; postgraduate loans funded by **Scotland** and **Northern Ireland** are not Plan 3 and need no separate parameter: they repay under the Plan 4 (Scotland) and Plan 1 (NI) systems at 9%, combined with the undergraduate debt (select `plan4` / `plan1`). The newer **Lifelong Learning Entitlement (LLE)** loans (courses from Jan 2027) and **Advanced Learner Loans** also carry no new parameters: they repay on **Plan 5** (post-Aug-2023) or Plan 2 (pre-2023) terms (select `plan5` / `plan2`). There is no individual "apprenticeship loan" — apprenticeships are funded via the employer's Apprenticeship Levy (0.5%), already modelled. See [GOV.UK — Repaying your student loan](https://www.gov.uk/repaying-your-student-loan/what-you-pay) and the [Education (Student Loans) (Repayment) (Amendments for Lifelong Learning) Regulations 2026](https://www.legislation.gov.uk/uksi/2026/857/made).
